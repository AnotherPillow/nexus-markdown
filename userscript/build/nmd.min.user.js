...
//@ts-ignore
let e=new window.showdown.Converter;var o=$(".wys-panel").htmlcode().replace(/<h1 ?(?:[a-z\-0-9_]+=?(?:['"][^"]+['"])?)?>(.*?)<\/h1>/g,'<font size="6">$1</font>').replace(/<h2 ?(?:[a-z\-0-9_]+=?(?:['"][^"]+['"])?)?>(.*?)<\/h2>/g,'<font size="5">$1</font>').replace(/<h3 ?(?:[a-z\-0-9_]+=?(?:['"][^"]+['"])?)?>(.*?)<\/h3>/g,'<font size="4">$1</font>').replace(/<h4 ?(?:[a-z\-0-9_]+=?(?:['"][^"]+['"])?)?>(.*?)<\/h4>/g,'<font size="3">$1</font>').replace(/<h5 ?(?:[a-z\-0-9_]+=?(?:['"][^"]+['"])?)?>(.*?)<\/h5>/g,'<font size="2">$1</font>').replace(/<h6 ?(?:[a-z\-0-9_]+=?(?:['"][^"]+['"])?)?>(.*?)<\/h6>/g,'<font size="1">$1</font>'),o=e.makeMarkdown(o);let t=document.createElement("dialog"),n=(t.innerHTML=EDITOR_DIALOG_INNER,t.id="markdown-editor-dialog",document.body.appendChild(t),t.querySelector("wc-monaco-editor"));console.log(n,n.editor),n.editor.setValue(o),n.editor.getModel().onDidChangeContent(()=>{var o=n.editor.getValue(),o=e.makeHtml(o);// but we want the full content
